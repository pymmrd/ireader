...
class MagicSpider(BaseSpider):
    ...
    def parse(self, response):
		...
		for tr in trs:
			...
			link = tds[0].select("child::a/@href").extract()
			link = link[0] if link else ''
			name = tds[0].select("child::a/text()").extract()
			name = name[0] if name else ''
			txt_link = tds[1].select("child::a/@href").extract()
			txt_link = txt_link[0] if txt_link else ''
			author = tds[2].select("child::a/text()").extract()
			author = author[0] if author else ''
			word_num = tds[3].select('text()').extract()
			word_num = word_num[0] if word_num else ''
