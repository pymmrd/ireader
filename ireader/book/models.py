...
class Category(models.Model):
    name = models.CharField(u'标题', max_length=50, unique=True)
    parent = models.ForeignKey('self', related_name='children',
                               verbose_name=u'父分类', null=True,
                               blank=True)
    ancestors = models.ManyToManyField('self', symmetrical=False,
                                       related_name="descendants",
                                       editable=False, db_index=True,
                                       verbose_name=u'子分类')
    meta_keywords = models.CharField(u'元关键字', max_length=255,
                                     blank=True)
    meta_description = models.CharField(u'元描述', max_length=255,
                                        blank=True)
